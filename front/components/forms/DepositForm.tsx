...
import { deposit } from "@/lib/actions/wallet";
import { CurrencyInput } from "@/components/CurrencyInput";

export function DepositForm() {
  const [state, formAction, pending] = useActionState(deposit, undefined);
  ...
  return (
    <form action={formAction} className="flex flex-col gap-3">
      <label htmlFor="depositAmount" className="text-sm text-zinc-600 dark:text-zinc-400">
        Valor
      </label>
      <CurrencyInput id="depositAmount" name="amount" />
